...
| Capability | Claude Code | Codex | OpenCode | Antigravity | Pi |
|---|---|---|-|---|--|
| Skills (SKILL.md native) | ✅ | ✅ | ✅ via `.opencode/skills/` | ✅ (native, self-contained per plugin) | ✅ (recursive discovery) |
| Subagents (markdown native) | ✅ | TOML format | ✅ (different frontmatter) | ✅ (`agents/<name>.md` + `invoke_subagent`/`define_subagent`) | via the reference `subagent` extension (`agents/<plugin>__<agent>.md`) |
...
| Plugin marketplace | ✅ | — | — | ✅ (`agy plugin install <name>@marketplace` / `agy plugin link`) | — (packages via npm, git, or a local path) |
| Parallel subagents | ✅ | ✅ | ✅ | ✅ | ✅ (extension) |
| Per-agent tool allowlist | ✅ (`tools:`) | only `sandbox_mode` | ✅ (`permission:` block) | ✅ (`tools:`, agy-native names) | ✅ (`tools:`, extension) |
| `TodoWrite` tool | ✅ | — | ✅ | — | — |
...
| Context file | `CLAUDE.md` | `AGENTS.md` (32 KiB cap) | `AGENTS.md` / `~/.claude/CLAUDE.md` | `AGENTS.md` (read natively) | `AGENTS.md` |
...
| Tool name case | CamelCase (`Read`) | action verbs (no tool vocab) | lowercase (strict) | lowercase (agy-native names) | lowercase (`read`, `bash`) |
| Bare model aliases | ✅ (`fable`/`opus`/`sonnet`/`haiku`) | mapped to GPT-5.x family | full provider/model-id | mapped to tier alias (`pro`/`flash`/`inherit`) | full provider/model-id |
...
- **Per-agent tool allowlist** — `tools:` frontmatter honored verbatim (Codex is coarser; the OpenCode adapter translates this into a `permission:` block).
- **`Task` / `Agent` spawn tool** — fan-out parallel subagent execution. (Codex requires naming an agent in prose to delegate.)
- **`TodoWrite`** — native progress tracking. (Not available in Codex / Antigravity / Pi.)
...
| Source pattern | Codex | OpenCode | Antigravity | Pi |
|---|---|-|---|---|
| `tools: Read, Grep` (agent allowlist) | dropped; `sandbox_mode = "read-only"` heuristic | converted to `permission:` deny block | rewritten to agy-native tool names | rewritten to Pi built-in names; `tools: []` becomes `read, grep, find, ls` |
| `color: blue` (agent) | dropped | dropped | dropped | dropped |
| `model: opus` (agent) | mapped to `gpt-5.5` | rewritten to `anthropic/claude-opus-4-8` | mapped to `pro` | rewritten to `anthropic/claude-opus-4-8` |
| `model: fable` (agent) | mapped to `gpt-5.5` | rewritten to `anthropic/claude-fable-5` | mapped to `pro` | rewritten to `anthropic/claude-fable-5` |
| `TodoWrite` in body | no equivalent — leave as-is | works as-is | no equivalent | no equivalent |
| Skill body > 8 KB | split into `references/details.md` | passed through | passed through | passed through |
| Agent named `worker` | namespaced to `<plugin>__worker` | passed through | passed through (no `<plugin>__` namespacing — the plugin dir already scopes it) | namespaced to `<plugin>__worker.md` (the agents directory is flat) |
| Slash command (`commands/<x>.md`) | converted to skill | rewritten to `.opencode/commands/` | TOML at `commands/<plugin>/<x>.toml`, body always inlined (never `@{path}`-injected) | prompt template at `prompts/<plugin>__<x>.md`, the body is copied as written once Claude tool references are rewritten to Pi names, `$ARGUMENTS` is left in place for Pi to substitute, and no wrapper text is added |
...
generated Codex, OpenCode, Antigravity, Copilot, or Pi outputs.
